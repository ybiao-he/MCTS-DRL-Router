...
class CREnv(gym.Env):
    ...
    def getReward(self):
        ...
        if self.connection:
            return 20
        if self.collide:
            left_dist = 4*np.linalg.norm(np.array(self.last_node) - np.array(self.finish[self.pairs_idx-1]))
    ...
    def board_embedding(self):
        ...
        state = np.array(list(self.head)+list(self.pre_head)+dist_to_target)
