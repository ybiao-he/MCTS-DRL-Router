# The env in this script has 4 candudate actions: up, down, left and right
...
class CREnv(gym.Env):
    ...
    def reset(self):
        ...
        self.max_pair = int(max(self.start.keys()))
    ...
    def path_node_to_act_idx(self, path_node):

        # check if path node is legal
        d = np.array(path_node)-np.array(self.head)
        if tuple(d) not in self.directions:
            print("Given path node is not right, it can not generate an legal direction with {}".format(d))
            return None

        # compute direction/action idx
        d = tuple(d)
        act_idx = self.directions.index(d)
