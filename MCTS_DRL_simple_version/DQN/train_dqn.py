...
def build_q_network(n_actions, learning_rate=0.00001, input_shape=(84, 84), history_length=4):
    ...
    """
    model_input = Input(shape=(input_shape[0], input_shape[1], input_shape[2]))
    x = Lambda(lambda layer: layer / 255)(model_input)  # normalize by 255

    x = Conv2D(32, (5, 5), strides=2, kernel_initializer=VarianceScaling(scale=2.), activation='relu', use_bias=True)(x)
    x = Conv2D(32, (4, 4), strides=2, kernel_initializer=VarianceScaling(scale=2.), activation='relu', use_bias=True)(x)
